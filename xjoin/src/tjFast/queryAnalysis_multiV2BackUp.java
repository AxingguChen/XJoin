...
public class queryAnalysis_multiV2BackUp extends DefaultHandler{
            ...
            //else Result is not empty, find if any other added tags in Result occurs in tablesToMerge
                ...
                for(int addedTagCursor=0; addedTagCursor<addedTags.size(); addedTagCursor++){
                    ...
                    if(!addedTagColumn.isEmpty()){
                        ...
                        if(addTag.equals("asin")){
                            if( addedTagCursor == 1){
//                            List<Vector> myFile = tablesToMergeOnAddedTag.get(0);
//                            for(Vector l:myFile){
//                                try {
//                                    BufferedWriter out = new BufferedWriter(new FileWriter("xjoin/src/multi_rdbs/asin_orderId.txt",true));
...
//                            }
//
//                            for(Vector l:myResult){
//                                try {
//                                    BufferedWriter out = new BufferedWriter(new FileWriter("xjoin/src/multi_rdbs/myResult.txt",true));
//                                    out.write(l.get(0)+" "+l.get(2)+" "+l.get(4)+" "+ "\r\n");  //Replace with the string
//                                    //you are trying to write
//                                    out.close();
//                                }
//                                catch (IOException e)
...
//
//                                }}
                            List<Vector> myNew = new ArrayList<>();
                            for(int i=0;i < myResult.size(); i++){
                                Vector v = myResult.get(i);
                                if(v.get(2).equals("0")){
                                    List<String> myList = Arrays.asList("B001CXLSSW","B002P51NXQ","B00BC6036C","B00IL2QNQO","B00FR84QG4");
                                    for(int mm=0;mm<myList.size();mm++){
                                        Vector m = (Vector) v.clone();
                                        String myValue = myList.get(0);
                                        m.addAll(Arrays.asList(myValue, tagHashMap.get(myValue)));
...
//                                        m.addAll(Arrays.asList(myValue, tagHashMap.get(myValue)));
                                ...
                                if(v.get(2).equals("1")){
                                    ...
                                    List<String> myList = Arrays.asList("B001QBZIMC","B000UUBFZY","B0093Q8JCI","B004J41HQY");
                                    for(int mm=0;mm<myList.size();mm++){
                                        Vector m = (Vector) v.clone();
                                        String myValue = myList.get(0);
                                        m.addAll(Arrays.asList(v,myValue, tagHashMap.get(myValue)));
...
//                                        m.addAll(Arrays.asList(myValue, tagHashMap.get(myValue)));
                        ...
                        else if(addTag.equals("price")){
                            List<Vector> myNew = new ArrayList<>();
                            List<String> myList = Arrays.asList("B001CXLSSW","B002P51NXQ","B00BC6036C","B00IL2QNQO","B00FR84QG4","B001QBZIMC","B000UUBFZY","B0093Q8JCI","B004J41HQY");
                             for(int i=0;i < myResult.size(); i++){
                                Vector v = myResult.get(i);
                                if(myList.contains(v.get(6))){
                                    Vector m = (Vector) v.clone();
                                    String price = "37.94";
                                    m.addAll(Arrays.asList(price, tagHashMap.get(price)));
                                    myNew.add(m);
                                }
                            }
                            myResult = myNew;
...
//        basicDocuemnt = "xjoin/src/test.xml";
        ...
        xmlReader.setContentHandler(new queryAnalysis_multiV2BackUp());
