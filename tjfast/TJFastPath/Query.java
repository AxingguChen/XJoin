...
public class Query { 
	...
	static Hashtable  queryIgnoreWildcardsLastPart; // 这里的query 字符，我们忽略了所有的wildcards "*" and "//" , 
	                                         
	static void preComputing(DTDTable DTDInfor){ //这个方法是重要的，我们预先计算一些值然后就不用每次都重新再算一遍
  ...
  calculateNumberofNodes();
	...
	static String [] getPathNames (String leave){ // 这个方法主要用于近似匹配，返回字符名序列,没有 wildcards "//" and "*"
  ...
  static void calculateParentTable(){
	...
	static void calculateIgnoreWildcardLastTable(DTDTable DTDInfor){
	 ...
	 static int [] calculateIgnoreWildcardsLastPart(String leave,DTDTable DTDInfor){
	 	...
	 	for(int i=0;i<pathInt.length;i++)
	 			...
	 			//这里这样写 主要考虑到getPath返回的是从叶子到根，现在必须颠倒过来
	...
	static Vector getPath(String leave){ // 注意：getPath返回的顺序是从叶子到根!
	...
	static Vector  getPathPattern (String leave){ //这个方法显示整个路径,AD关系用string "//"来表示。
	...
	static boolean isTopBranch(String branch){
		...
		if (branch[1]==null)//如果仅有一个branch点作以下处理：
	...
	static void calculateBanchNodesInt(DTDTable DTDInfor){ // 这里返回整数的branching节点的值
	...
	//主要用来计算branchpoint的偏移量，key 是String, 格式是branch node%leaf node
	//object是int [2]， 第一个是在哪个分解的pattern中，第二个是位移偏量
